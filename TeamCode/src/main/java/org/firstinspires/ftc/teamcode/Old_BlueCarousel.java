...
public class Old_BlueCarousel extends LinearOpMode {
    ...
    public void deliveryMovement() {
        gyroCorrect();

        Trajectory strafeCarousel = vector.trajectoryBuilder(new Pose2d(0, 0, 0))
                .lineToLinearHeading(new Pose2d(7.5, 15, Math.toRadians(0)))
                .build();
        vector.followTrajectory(strafeCarousel);
        gyroCorrect();

        carouselDelivery();
        gyroCorrect();

        Trajectory strafeHub = vector.trajectoryBuilder(new Pose2d(0, 0, 0))
                .lineToLinearHeading(new Pose2d(-25, -25, Math.toRadians(0)))
                .build();
        vector.followTrajectory(strafeHub);
        gyroCorrect();
        sleep(100);
        Trajectory strafeWall = vector.trajectoryBuilder(new Pose2d(0, 0, 0))
                .lineToLinearHeading(new Pose2d(15, -15, Math.toRadians(0)))
                .build();
        vector.followTrajectory(strafeWall);

    }

    public void parkingMovement(){
        gyroCorrect();
        Trajectory backWarehouse = vector.trajectoryBuilder(new Pose2d(0, 0, 0))
                .lineToLinearHeading(new Pose2d(25, -25, Math.toRadians(0)))
                .build();
        vector.followTrajectory(backWarehouse);
        gyroCorrect();

        Trajectory leftWarehouse = vector.trajectoryBuilder(new Pose2d(0, 0, 0))
                .lineToLinearHeading(new Pose2d(-30, -45, Math.toRadians(0)))
                .build();
        vector.followTrajectory(leftWarehouse);
        gyroCorrect();

    }

    public void freightTop() {

        armRotationalAxis(0.75, -10);
        armRevolutionaryAxis(0.75, -20);
        sleep(1000);
        gyroCorrect();

        Trajectory forwardHubPos = vector.trajectoryBuilder(new Pose2d(0, 0, 0))
                .lineToLinearHeading(new Pose2d(-14, 14, Math.toRadians(0)))
                .build();
        vector.followTrajectory(forwardHubPos);
        gyroCorrect();

      //  vector.intake.setPower(0.75);
        sleep(2500);
      //  vector.intake.setPower(0);
    }

    public void freightMiddle() {
        armRevolutionaryAxis(0.75, -20);
        sleep(1000);
        gyroCorrect();
    ...
    public void freightBottom() {
        armRevolutionaryAxis(0.75, -20);
        armRotationalAxis(-0.75, 5);
        sleep(1000);
        gyroCorrect();
    ...
    public void carouselDelivery()
    {
      //  vector.carousel.setVelocity(200);
        sleep(2500);
      //  vector.carousel.setVelocity(0);
       // vector.carousel.setPower(0);
    }

    public void armRotationalAxis (double velocity, int position) {
        //reset encoder
       // vector.armRotation.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
        //set target position
       // vector.armRotation.setTargetPosition(position * 75);
      //  vector.armRotation.setMode(DcMotor.RunMode.RUN_TO_POSITION);
        //set power
        //vector.armRotation.setPower(velocity);

     //   while (vector.armRotation.isBusy()){

      //  }
      ////  vector.armRotation.setPower(0);
    }

    public void armRevolutionaryAxis (double velocity, int position) {
        //reset encoder
       // vector.armRevolution.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
        //set target position
       // vector.armRevolution.setTargetPosition(position * 75);
      //  vector.armRevolution.setMode(DcMotor.RunMode.RUN_TO_POSITION);
        //set power
      //  vector.armRevolution.setPower(velocity);

      //  while (vector.armRevolution.isBusy()){

      //  }
      //  vector.armRevolution.setPower(0);
    }
